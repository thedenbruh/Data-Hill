...
//test
  ...
  '</script>' => 'NO',
  '<script>' => 'haha i am funny js guy who wants to window.location.replace the whole Data Hill server',
    );
...
<!DOCTYPE html>
  <head>
    <title><?php echo $userRow['username']; ?> - Data Hill</title>
  <meta charset="UTF-8">
  <meta name="description" content="<?php echo $userRow['username'] ?> - Data Hill User">
  <meta name="keywords" content="free,game">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  
  <meta property="og:title" content="<?php echo $userRow['username']; ?> - Data Hill User" />
...
if($loggedIn) {
        ...
        $AlreadyFriends = mysqli_num_rows($AlreadyFriendsQ);
            ...
            if($AlreadyFriends<1){
              echo '<a href="/user/friends/add?id=' . $id . '"><input class="blue-button" type="button" value="Add Friend"></a><a style="padding-left:2px;" href="/report?type=user&id='.$userRow['id'].'"><input class="red-button" type="button" value="Report"></a>';
            } else {
              echo '<a href="/user/friends/remove?id=' . $id . '"><input class="red-button" type="button" value="Unfriend"></a><a style="padding-left:2px;" href="/report?type=user&id='.$userRow['id'].'"><input class="red-button" type="button" value="Report"></a>';
            }
		...
		echo '<ul>
				...
				<li><span style="font-weight:bold;">Last Online: </span>' . $timedif . ' ago
					...
					else {        //using Data Hill:revived source is fire
...
<?php 
		include('ucurr.php'); //ugly hack, but it works
?>
...
<script>
  ...
  function getPage(type, page) {
    $("#crate").load("/user/crate?id="+id+"&type="+type+"&page="+page);
  };
