<?php 
  ...
  if (isset($_POST['submit'])) {
    ...
    if($mailCheck->num_rows >= 25) {
      $error[] = 'You can only associate 25 accounts with one email';
    }
    ...
    //important
    $checkUsernameSQL = "SELECT * FROM `tdbdnh`.`beta_users` WHERE `beta_users`.`usernameL` = '$usernameL'"; //fixed
...
<br><h6 style="font-size:16px;">By signing up, you agree to the <a href="/terms/">Terms of Service</a></h6>
        ...
        <div style="margin:10px;border:1px solid #000;padding:10px;background-color:#FFF;clear:right;float:right;width:300px;">
          <h4>Under 13s...</h4>
          <p>If you are under 13..... i forgot</p>
        </div>
        <div style="margin:10px;border:1px solid #000;padding:10px;background-color:#FFF;clear:right;float:right;width:300px;">
